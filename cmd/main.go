...
func main() {
	...
	userService := service.NewUserService(userRepo, ownerRepo, firebaseApp.Auth)
	...
	contractUsageRepo := repository.NewContractUsageRepository(pool)
	contractExtractService := service.NewContractExtractService(cfg.OpenAIAPIKey, cfg.OpenAIModel)
	contractHandler := handler.NewContractHandler(contractExtractService, contractUsageRepo)
